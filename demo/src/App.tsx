...
import {
  ...
  rgbToString,
  calculateBrightness,
} from "@niraj/image-dominant-hover";
import "./App.css";
  ...
  return (
    <div className="app">
      ...
      <section className="demo-section">
          ...
          {sampleImages.map((image, index) => (
            <ImageCard
              ...
              onColorExtractionError={(error) => {
                console.error(
                  `Failed to extract color for ${image.title}:`,
                  error
                );
              }}
              className="demo-card"
        ...
        {clickedCard && (
          <div className="notification">✨ Clicked: {clickedCard}</div>
        )}
      </section>
          ...
          <div className="info-card color-info">
            ...
            <div className="color-display">
              ...
              <div className="color-details">
                <p>
                  <strong>RGB:</strong> {rgbToString(selectedColor)}
                </p>
                <p>
                  <strong>Values:</strong> R:{selectedColor.r} G:
                  {selectedColor.g} B:{selectedColor.b}
                </p>
                <p>
                  <strong>Brightness:</strong>{" "}
                  {(calculateBrightness(selectedColor) * 100).toFixed(1)}%
                </p>
              </div>
            </div>
          </div>
